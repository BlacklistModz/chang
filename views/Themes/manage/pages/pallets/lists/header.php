	...
	<div ref="actions" class="listpage2-actions">
		<div class="clearfix mbs mtm">
			...
			<ul class="lfloat" ref="actions">
				<li class="mt">
					<h2><i class="icon-<?=$this->item['icon']?> _ico-center mrs"></i><span><?=$this->item['name']?> (<?=$this->item['code']?>)</span></h2>
				</li>
				...
				<li class="divider"></li>
				 ...
				 <!-- data-plugins="dialog" -->
				<li class="mt"><a class="btn btn-blue" href="<?=URL?>pallets/add?type=<?=$this->item['id']?>"><i class="icon-plus mrs"></i><span><?=$this->lang->translate('Add New')?></span></a></li>
		...
		<div class="clearfix mbl mtm">
			<ul class="lfloat" ref="control" data-plugins="search_Pallets" data-options="<?=$options = $this->fn->stringify( array('type'=>$this->item['id']) )?>">
				...
				<li>
					<label for="size" class="label">SIZE (ขนาด)</label>
					<select ref="selector" name="size" class="inputtext" data-name="size">
						<option value="">-</option>
						<?php
				...
				<li>
					...
					<select ref="selector" name="weight" class="inputtext" data-name="weight">
						<option value="">-</option>
					</select>
				</li>
				<li>
					<label for="grade" class="label">GRADE</label>
					<select ref="selector" name="grade" class="inputtext">
						<option value="">-</option>
						<?php
						foreach ($this->grade['lists'] as $key => $value) {
							echo '<option value="'.$value['id'].'">'.$value['name'].'</option>';
						}
						?>
					</select>
				...
				<li>
					<label for="warehouse" class="label">Zone (โกดัง)</label>
					<select ref="selector" name="warehouse" class="inputtext" data-name="warehouse">
						<option value="">-</option>
						<?php
				...
				<li>
					<label for="rows" class="label">แถวที่</label>
					<select ref="selector" name="rows" class="inputtext" data-name="rows">
						<option value="">-</option>
					</select>
				</li>
			</ul>
