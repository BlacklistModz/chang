...
class trucks extends Controller {
    ...
    public function add() {
      ...
      $this->view->setData('name', $this->model->query('trucks')->lists());
      $this->view->setData('address', $this->model->query('trucks')->lists());

      // $this->view->setData('status', $this->model->status());

      $this->view->setPage('path','Forms/trucks');
    ...
    public function save() {
        ...
        try {
            ...
            $form   ->post('name')->val('is_empty')
                    ->post('license_plate')->val('is_empty')
                    ->post('brand')->val('is_empty')
                    ->post('detial');
            ...
            if( empty($arr['error']) ){
                ...
                $arr['url'] = URL.'settings/planload/trucks';
            }
